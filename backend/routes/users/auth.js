...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    await sql`INSERT INTO users2 (firstname, lastname, email, role, created_at, last_login, firebase_uid)
    VALUES (${firstname}, ${lastname}, ${email}, ${role}, NOW(), NOW(), ${firebaseUId})`;
    // Renvoi du user nouvellement créé
    const newUser = await sql`SELECT * FROM users2 WHERE firebase_uid = ${firebaseUId}`
